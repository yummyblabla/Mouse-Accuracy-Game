...
const drawStaticUI = () => {
	...
	const survivalModeButton = () => {
		...
		let lines = text.split('\n');
		for (let i = 0; i < lines.length; i++) {
			ctx.fillText(lines[i], buttons.survival.textX, canvas.height - 50 + (i * buttons.lineHeight));
		}
	}
	...
	const addMonstersKilled = () => {
		...
		ctx.textAlign = "right";
		ctx.textBaseline = "top";
		ctx.fillText(`Monsters Killed: ${monstersKilled}` , canvas.width, canvas.height - statsFont.fontSize);
	}
